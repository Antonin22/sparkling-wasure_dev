...
function run_fun () {
	...
	if [[ "$(docker images -q $NAME_IMG 2> /dev/null)" == "" ]]; then
	    ...
	    COLS=$([ -t 0 ] && tput cols || echo 200)
	    LINES=$([ -t 0 ] && tput lines || echo 50)
	    CMD="docker run  -d $MOUNT_CMD -u 0 --cap-add SYS_ADMIN  --privileged --net host $PROXY_LINE -e DDT_MAIN_DIR='$DDT_MAIN_DIR_DOCKER' -e COLUMNS=$COLS -e LINES=$LINES --name $CONTAINER_NAME  -i $TTY_FLAG ${NAME_IMG}"
	    eval $CMD 
